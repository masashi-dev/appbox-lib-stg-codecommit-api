package jp.co.fnj.storage.api.logic.foldercheck;
...
import jp.co.fnj.storage.api.service.foldercheck.FolderNameCheckService;

/**
 * 論理フォルダ名重複チェックAPIサービス.
 *
 * @param <REQUEST_BODY>
 * @param <RESPONSE>
 */
@Service
public class FolderNameCheckLogic<REQUEST_BODY extends FolderNameCheckRequest, RESPONSE extends FolderNameCheckResponse> {

  @Autowired
  private FolderNameCheckService<REQUEST_BODY, RESPONSE> folderNameCheckService;
  ...
  private RESPONSE innerExecute(HttpServletRequest request, HttpServletResponse response,
    ...
    try {
      // 各種サービスを順次実行
      RESPONSE res = (RESPONSE) folderNameCheckService.execute(request, response, requestBody);
      return res;
    } catch (Exception e) {
