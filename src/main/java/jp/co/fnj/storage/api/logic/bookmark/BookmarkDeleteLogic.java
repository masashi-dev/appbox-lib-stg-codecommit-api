...
import jp.co.fnj.storage.api.service.bookmark.BookmarkDeleteService;
 ...
 */
@Service
public class BookmarkDeleteLogic<REQUEST_BODY extends BookmarkDeleteRequest, RESPONSE extends BookmarkDeleteResponse> {
  @Autowired
  private BookmarkDeleteService<REQUEST_BODY, RESPONSE> bookmarkDeleteService;
  ...
  private void innerExecute(HttpServletRequest request, HttpServletResponse response,
    ...
    try {
      // 各種サービスを順次実行
      bookmarkDeleteService.execute(request, response, requestBody);
