...
import jp.co.fnj.storage.api.service.bookmark.BookmarkGetService;
 ...
 */
@Service
public class BookmarkGetLogic<REQUEST_BODY extends BookmarkGetRequest, RESPONSE extends BookmarkGetResponse> {
  @Autowired
  private BookmarkGetService<REQUEST_BODY, RESPONSE> bookmarkGetService;
  ...
  private RESPONSE innerExecute(HttpServletRequest request, HttpServletResponse response,
    ...
    try {
      // 各種サービスを順次実行
      RESPONSE res = (RESPONSE) bookmarkGetService.execute(request, response, requestBody);
      return res;
    } catch (Exception e) {
      throw new StorageRuntimeException(Messages.E04008, e);
    }
  }
