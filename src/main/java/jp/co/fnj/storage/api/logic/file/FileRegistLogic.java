...
import jp.co.fnj.storage.api.service.file.FileRegistService;
 ...
 */
@Service
public class FileRegistLogic<REQUEST_BODY extends FileRegistRequest, RESPONSE extends FileRegistResponse> {

  @Autowired
  private FileRegistService<REQUEST_BODY, RESPONSE> fileRegistService;
  ...
  private RESPONSE innerExecute(HttpServletRequest request, HttpServletResponse response,
    ...
    try {
      // 各種サービスを順次実行
      RESPONSE res = (RESPONSE) fileRegistService.execute(request, response, requestBody);
      return res;
    } catch (Exception e) {
